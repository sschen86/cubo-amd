(function (exports) {
    ...
    function amd (config) {
        ...
        const configCuboUrl = config.cuboUrl

        if (/^https?:\/\//.test(configCuboUrl)) { // 绝对路径
            cuboUrl = configCuboUrl
        } else {
            const currentPath = window.location.href.split('#')[0].replace(/[^/]+$/, '')
            cuboUrl = configCuboUrl ? pathNormalize(currentPath + configCuboUrl + '/') : currentPath + 'cubo-modules/'
        }

        // console.info({ cuboUrl, baseUrl })
    ...
    function defineModule (id, dependencies, factory) { // 定义模块
        id = id || getModuleId()
        if (modules[id]) {
            return console.warn('module "' + id + '" has allready define!')
        }
    ...
    function getModuleId (id, parentId) {
        let moduleId
        if (id == null) {
            const currentSrc = document.currentScript.src.replace('.js', '')
            if (currentSrc.indexOf(baseUrl) === 0) {
                moduleId = currentSrc.replace(baseUrl, './')
            } else {
                moduleId = currentSrc.replace(cuboUrl, '')
            }
        } else if (/^[\w]/.test(id)) { // cubo-modules
            moduleId = id.indexOf('/') === -1 ? id + '/index' : id
        } else {
            moduleId = pathNormalize(parentId.replace(/[^/]+$/, '') + id)
        }
    ...
    function getModuleDependencies (factory, parentId) {
        const dependencies = []
        String(factory).replace(/\/\/.+|\/\*[^]*\*\//g, '').replace(/require\(\s*(['"])([^'"]+)\1\s*\)/g, (match, quote, id) => {
    ...
    function loadModule (id, parent, retry = 2, callback) { // 加载失败重试3次
        ...
        function retryLoad (retry) {
            setTimeout(() => {
                delete loadingModules[id]
                if (retry > 0) {
                    loadModule(id, parent, retry - 1, callback)
                } else {
    ...
    function getModuleUrl (id) {
        // console.info({ getModuleUrl: id })
        if (/^[\w]/.test(id)) {
            if (/\//.test(id)) {
                return pathNormalize(cuboUrl + id) + '.js'
            } else {
                return pathNormalize(cuboUrl + id + '/index') + '.js'
            }
        } else {
